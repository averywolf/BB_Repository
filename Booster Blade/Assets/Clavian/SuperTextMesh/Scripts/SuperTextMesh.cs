	...
	private string FlipParagraphs(string myText, bool flipInfo)
	{
		...
		for(int i=0; i<myText.Length; i++) //manually go through every letter in the string, looking for end of paragraph.
		{
			...
			else //add to existing word.
			{
				...
				{
					paragraphs[paragraphIndex][wordIndex] += myText[i].ToString();
				}
...
//		}
	...
	*/
	...
	void CalculateLineHeights()
	{
		...
		float biggest = infoCount > 0 ? info[0].size.y * lineSpacing : size * lineSpacing; //just in case this gets called somehowo
...
Vector3 JitterValue_MyJit = Vector3.zero;
   ...
   //--------------------------------------------------
	...
	{ //set the data for the endmesh
		...
		for(int i=0, iL=hyphenedText.Length; i<iL; i++)
		{
			...
			UpdateMesh_lowestLineOffset.y += anchorOffset.y + ((lineSpacing - 1f) * info[0].size.y); //push up to hide line spacing on first row.
	...
	*/
	...
	private void ApplyFade(Color32[] colorArray, float fadeAmount)
	{
		for(int i=0; i<colorArray.Length; i++)
		{
			colorArray[i].a = (byte)((float)colorArray[i].a * fadeAmount);
		}
	}
